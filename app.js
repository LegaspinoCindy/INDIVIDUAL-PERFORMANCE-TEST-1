...
// Endpoint to retrieve all backend courses sorted alphabetically
app.get('/api/backend-courses', async (req, res) => {
    try {
      // Filter courses data to include only backend courses
      ...
      // Initialize array to store all backend courses
      let allBackendCourses = [];
  
      // Concatenate backend courses from filtered data
      filteredCourses.forEach(year => {
        Object.values(year).forEach(courseList => {
          allBackendCourses = allBackendCourses.concat(courseList.filter(course => isBackendCourse(course)));
        });
      });
  
      // Sort all backend courses alphabetically by description
      allBackendCourses.sort((a, b) => a.description.localeCompare(b.description));
  
      // Send sorted backend courses as JSON response
      res.json(allBackendCourses);
    } catch (error) {
      // Handle errors by logging and sending internal server error response
      console.error('Error:', error);
      res.status(500).json({ error: 'Internal server error' });
    }
});
